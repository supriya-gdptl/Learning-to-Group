...
def save_open3d_images_of_pointcloud(category, level, point_cloud_idx, output_folder):
    """
    save renderings of an object and predicted parts
    :param category: (string) shape category
    :param level: (int) level of part segmentation granularity (level=1: coarser, level=3: finer)
    :param point_cloud_idx: (int) index of shape to save renderings of
    :param output_folder: (string) folder location to save output renderings
    :return:
    """
    ...
    # open predicted part segmentation
    predicted_mask_path = f"../pretrained_results/{category}/Level_{level}/test-00.h5"
...
if __name__ == '__main__':
    save_open3d_images_of_pointcloud(category="Bag", level=1, point_cloud_idx=0, output_folder="../../../www/partnet/pretrained_prediction")
